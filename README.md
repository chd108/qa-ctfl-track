...
| Pfad | Beschreibung |
|------|----------------|
| `index.html` | Einstiegsseite — **17 Karten** zu allen aktiven Lernmodulen |
| `pages/` | Einzelne Lernmodule (siehe Tabelle unten) |
| `assets/css/` | `base.css`, `subpage.css` — Hauptsystem; `doc-a4.css` — **A4-/Dokumentmodus** (04b, 04d, 05b); `index.css` — Einstieg; `formular-base.css` — ältere Formularbasis (liegt im Repo, wird von den aktuellen HA-A4-Seiten **nicht** eingebunden) |
...
**Kurzüberblick (Stand 21.03.2026):** V2-Hauptsystem und Benennungsmatrix sind umgesetzt; **02c** ist migriert; die **HA4/HA5-Lernstrecke** (04a–05b) ist inhaltlich und formal gehärtet — u. a. einheitliche **Meta-Informationen** (Status, Daten, Features), **Dokumentenkette** zwischen Web- und A4-Fassungen, konsistente Terminologie **Testbasis-Analyse**, sowie A4-Ergänzungen in **`doc-a4.css`** (z. B. Status-Badge für Review). Details und Commit-Historie: **Git-Repository `QA_Lernwebseite`**.
