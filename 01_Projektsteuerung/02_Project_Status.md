...
| **Website *QA_Lernwebseite*** | **Technische Basis abgeschlossen** — einheitliches Hauptsystem, `index.html` als alleiniger Einstieg, Benennungslogik inkl. Matrix für aktive Seiten umgesetzt; **IA (Stand 2026-04-11):** HTML unter **`pages/01-lerninhalte/`**, **`pages/02-hausaufgaben/`**, **`pages/03-lektionen/`**; **Kopfnavigation (Stand 2026-04-02, ergänzt 2026-04-11):** **Startseite** als **Logo** im ersten Listenpunkt der **Lernmaterial**-Nav (Favicon-Asset, **`--nav-inline-logo-height`**), kein zweites Logo links außerhalb der Leisten; **dritte Nav-Leiste** *Lernzusammenfassung* (`main-nav--summary`) und **dritte Kartenzeile** auf **index** (Start: **`pages/03-lektionen/07-lernzusammenfassung.html`**); **2026-04-11:** alle drei Nav-Zeilen **gleiche** Textlink-Typo/Farbe (`base.css`), vertikaler Rhythmus **`--nav-vertical-rhythm: 1.25rem`**; **Lernzusammenfassung** Interview-Kästen **Kap. 5–7** mit Querverweisen (Lerninhalte, Lektionen, HA); **Designsystem-Pilot:** **F-META** / **F-LEGEND** / **F-CHAIN** (Dokumentenkette, **creme/pastell**, **04d** B.13) / Korrekturkomponenten in **`base.css`**, **meta-bar** auf **11** Kernseiten; **K-Stufen**-Kurzzeile im **Hinweis** nur wenn die **meta-bar** bereits **(K1)/(K2)/(K3)** oder **FL-… K1/K2/K3** enthält (**9** Seiten); **SDLC**-Seiten **02a/02b** mit Triptych-Layout und **1400px** Inhaltsbreite. |
...
| **Hausaufgabe HA6 (Testdurchführung)** | **Website-Portfolio:** Testprotokoll, Fehlerbericht und Testabschlussbericht als **A4-Seiten** unter `02_Portfolio/QA_Lernwebseite/pages/02-hausaufgaben/` (**06a–06c**) **vorhanden**; **Kursordner** `03_Hausaufgaben/HA6_Testdurchfuehrung/` ggf. separat nach Kursvorgaben (siehe Abschnitt 3). |
...
- **Navigation:** **`base.css`** — **`.main-nav--summary`**; **`index.html`**, **HTML unter `pages/`** (Unterordner **`01-lerninhalte/`**, **`02-hausaufgaben/`**, **`03-lektionen/`**), **`templates/referenzvorlage-hauptsystem.html`** — **dritte** `<nav>` unter **Lernmaterial** / **Hausaufgaben** / **Lernzusammenfassung** (`aria-label="Lernzusammenfassung"`).  
- **Einstieg:** **`index.html`** — **dritte** Kartenzeile unter Lernmaterial/Hausaufgaben (Abstand wie Orientierung → Lernmaterial; Kartenfarbe Kopf **`--surface-loesung-kopf-bg`** wie HA3-Lösungskopf; Raster **6** Spalten für gleiche Kartenbreite wie oben).  
- **Seite:** [`07-lernzusammenfassung.html`](../02_Portfolio/QA_Lernwebseite/pages/03-lektionen/07-lernzusammenfassung.html) — Hauptsystem (Skip-Link, **page-chrome**, **meta-bar**, Footer); **inhaltlich** Sidebar + Kapitelkarten unverändert.  
- **Doku:** [README.md](../02_Portfolio/QA_Lernwebseite/README.md) (Seitenliste, Nav-Beschreibung); **[03_Project_Standards.md](03_Project_Standards.md)** §6/§7; dieser Status (§1/§2).  
- **In Klartext:** Die Website hat einen **eigenen Einstiegsbereich** für **Kursfolien / Lernzusammenfassung** — **sichtbar** in Kopf **und** auf der Startseite; weitere Lektionen können in derselben Nav/Zeile ergänzt werden.

**Nachzug 2026-04-11 (Feinabgleich UI & Didaktik):**

- **`base.css`:** Drei Kopf-`<nav>`-Zeilen **einheitlich** (Schriftgröße **0,875rem**, Linkfarbe **`var(--soft)`**); spezifische abweichende Regeln für Hausaufgaben-/Summary-Zeile entfallen; vertikaler Abstand **`--nav-vertical-rhythm: 1.25rem`**.  
- **`07-lernzusammenfassung.html`:** Interview-Kästen **Kap. 5–7** — Kap. **5** fachlich präzisiert (Lektion vs. Portfolio **18+**); Kap. **6**/**7** um **Lerninhalt-**, **Lektions-** und **HA-Links** ergänzt (u. a. HA5 Testkonzept, HA6-Kette).  
- **Repo-Doku:** [README.md](../README.md) (Root), [README QA_Lernwebseite](../02_Portfolio/QA_Lernwebseite/README.md), **[03_Project_Standards.md](03_Project_Standards.md)** Metazeile — Stichtag **11.04.2026**.  

**In Klartext:** Kopfnavigation wirkt **überall gleich** (bis auf Home-Logo); die **Lernzusammenfassung** verbindet **Buch/Kurs** mit **eigenen Hausaufgaben** über **klickbare** Verweise — passend zu Charter (Portfolio, CTFL).

### 2026-04-05 — *QA_Lernwebseite* / HA3 **04e**: Seite nach Content-Pilot neu aufgebaut

- **Seite:** [`04e-ha3-ueberdeckung.html`](../02_Portfolio/QA_Lernwebseite/pages/02-hausaufgaben/04e-ha3-ueberdeckung.html) — **Inhalt** wortgetreu [pilot-ha3-content.txt](designsystem-workshop/pilot-ha3-content.txt); **Struktur und Darstellung** gemäß [pilot-ha3-content-phasen.md](designsystem-workshop/pilot-ha3-content-phasen.md) (**Phasen 0–6**): **`ha3p-*`** im Seiten-`<style>` (**base.css**-Tokens), Hauptabschnitte **Aufgabenstellung** / **Lösung**; **Sprungmarken** `#aufgabenstellung` / `#loesung`. **Kurs-Notion-Links** in **meta-bar** und im **Intro** unter der Aufgabenstellung (**kein** eigener **H2 „Quellen“**). **Farben:** [03_Project_Standards.md](03_Project_Standards.md) §6 · [P03 §2](designsystem-workshop/P03-mapping-ist-soll-token.md).
...
- **In Klartext:** Die **öffentliche HA3-Überdeckungs-Seite** ist nicht nur mit Diagramm **befüllt**, sondern **einheitlich** nach dem **Workshop-Pilot** gebaut — inklusive **Platz sparendem** zweispaltigen **Lösungs**-Layout wie bei **Code \| Testfälle** in der Aufgabenstellung.

### 2026-04-04 — *QA_Lernwebseite* / HA3: Diagramm-SVG unter `assets/ha3-svg/`, Doku nachgezogen

- **Asset:** `HA3-Teil2_DAG.svg` — Ablage **`02_Portfolio/QA_Lernwebseite/assets/ha3-svg/`** (statt Verweis aus `03_Hausaufgaben/…` über `../../../`), Einbindung in **`pages/02-hausaufgaben/04e-ha3-ueberdeckung.html`** mit **`../../assets/ha3-svg/HA3-Teil2_DAG.svg`**.
...
- **Landkarte** `pages/01-lerninhalte/01-landkarte.html`: Kap. **6** mit Syllabus-Stichpunkten **6.1** und **6.2** (Nutzen/Risiken); **C-PRAXIS**-Texte **kurz**, Konvention **Ausführlich (Kürzel)** (u. a. **DoD**, **UAT**, **PR**); **SonarQube (Werkzeug, Kap. 6)** in Kap. **3**, Werkzeugliste Kap. **6**; **Smoke-Tests** als **C-SYLLABUS** **`ref` 5.1.7** in **5.1** und **C-PRAXIS** **Smoke-Tests (Build-Gate)** in Kap. **2**; **FMEA** als Advanced **Langform (Kürzel)**; **`base.css`:** vertikaler Abstand `.tag-group .tags + .tags` (Syllabus- vs. Praxis-Zeile).
...
- **Landkarte** `pages/01-lerninhalte/01-landkarte.html`: **Kap. 1–6** gleicher **Kopf** (`section__header--landkarte-leit`, `section__sub--landkarte-leit`); **`section__sub--emph`** entfällt. **5.3:** drei **TOC**-Chips **5.3.1–5.3.3** unter Gruppe **5.3**.
...
- **Landkarte** `pages/01-lerninhalte/01-landkarte.html`: Kap. **5** — **5.1**/**5.2** **wortgleich** Lehrplan-Unterüberschriften; **5.4**/**5.5** mit **`ref`**. **Smoke-Tests:** später **C-SYLLABUS** **`ref` 5.1.7** in **5.1** ergänzt (siehe Eintrag **Landkarte — Praxis-Chips harmonisiert**).
...
- **Glossar** `pages/01-lerninhalte/01a-glossar.html`: Abschnitt Kap. **4** — Überschrift **„Testanalyse und -entwurf“** (wie Lehrplan/Landkarte).
...
- **Landkarte** `pages/01-lerninhalte/01-landkarte.html`: Kap. **2** an **ISTQB CTFL v4.0.2** TOC (Abschnitte **2.1–2.3**); Kapitelüberschrift **„Testen während des SDLC“**; Legende **drei** Inhaltsfarben (Syllabus / Praxis / Vertiefung), **MS** orthogonal; Chips nach **P04-04** **A.0.1c**; Kopf-Muster siehe **2026-04-03**.
...
- **Commit `95d6f18`** (Branch **`main`**, Remote **`origin`):** **Kein** separates Logo mehr links neben den Nav-Leisten; **Startseite** erscheint in **`main-nav--learn`** als **erster Listenpunkt** mit **Logo** (`class="main-nav__home"`, Bild **`qa-favicon-256.png`**, CSS-Variable **`--nav-inline-logo-height`**, u. a. **2rem**); **`base.css`** — zentrierte **`.top-nav-zone__brand-row`**, **`align-items: center`** auf **`.main-nav ul`**; **`README.md`** der Lernwebsite an den Ist-Stand angepasst; **`index.html`**, **22** Inhaltsseiten unter **`pages/`** (flache Ablage, Stand Commit), **`templates/referenzvorlage-hauptsystem.html`** konsolidiert. *Nachzug 2026-04-11:* **33** HTML-Dateien in **`pages/01-lerninhalte/`**, **`pages/02-hausaufgaben/`**, **`pages/03-lektionen/`**.
...
- **Design-Spec-Cluster (Workshop 04):** [P04-00-spec-overview.md](designsystem-workshop/P04-00-spec-overview.md) — **Risiko-Register**, **Component-Spec-Checkliste**, **04a–c** (C/F/P Priorität); **CTFL-Domain** [P04-04-ctfl-lernwebsite-spec.md](designsystem-workshop/P04-04-ctfl-lernwebsite-spec.md) — Syllabus-Chip / **F-META**, **B.11** (visuelle Sollwerte); **Referenzseite** `pages/01-lerninhalte/01c-testmittel.html`. **Pilot P1** kann starten (siehe §1 Dashboard).
...
- **Session 2c** ([P02-03-seitenmatrix-ist.md](designsystem-workshop/P02-03-seitenmatrix-ist.md)): **23 Seiten** (`index.html` + `pages/*.html`) manuell matrixartig erfasst — **IST**-Basis für Mapping. *(Hinweis 2026-04-11: Im Repo liegen die Inhalts-HTML-Dateien unter **`pages/01-lerninhalte/`** usw.; die Matrix nutzt weiterhin **Dateinamen** ohne Unterordner — inhaltlich dieselben Seiten.)*
