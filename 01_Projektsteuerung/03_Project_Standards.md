...
Letzte Aktualisierung: 2026-04-12 — **Doku:** **Standards-Seite** (`pages/01-lerninhalte/00b-standards.html`) als **Living Doc** / öffentlicher Ist-Spiegel in **§6** verankert (README Projektsteuerung, **QA_Lernwebseite**, **02_Project_Status**). Zuvor **2026-04-11** — **QA_Lernwebseite** — **§6** um **CSS-Schichten, Body-Basis, Breakpoints** (Folien- und Unterseiten-Refactoring) ergänzt. Zuvor: **dritte Kopf-Nav** *Lernzusammenfassung* (`main-nav--summary`), **dritte Kartenzeile** auf **`index.html`**, Seite **`pages/03-lektionen/07-lernzusammenfassung.html`**; **§6** / **§7** (gemeinsamer Rahmen, Checkliste Nav/Index) nachgezogen. **Nachzug:** **`base.css`** — einheitliche Textlink-Typo/Farbe für **alle drei** Kopf-`<nav>`-Zeilen; **`--nav-vertical-rhythm: 1.25rem`**; **`07-lernzusammenfassung.html`** — Interview-Kästen **Kap. 5–7** (Querverweise Lerninhalte, Lektionen, HA5/HA6). Zuvor **2026-04-05** **HA3** **04e** (`pages/04e-ha3-ueberdeckung.html`): Content-Pilot **0–6**; Web-SVG **`assets/ha3-svg/`**; **HA-Farbregel** **§6** / **P03** §2. Zuvor **2026-04-04** …  
...
- **Standards-Seite (`pages/01-lerninhalte/00b-standards.html`):** öffentliche **Checklisten-/Ist-Übersicht** (Web- und Repo-Standards); **normativ** für Regeln und Workflow bleibt **dieses Dokument** und die **Project\_*-Serie** — die Seite ist bewusst ein **Living Doc**-Spiegel, keine zweite Wahrheit.
...
| **`subpage.css`** | Layout-Muster **Seitenkopf** (`header`, `h1`, Untertitel) und **Fuß** (`.source-note`); **globale `body`-Regel** für Standard-Unterseiten (`padding`, `max-width`, `margin`) | **Schicht** für ein wiederkehrendes Unterseiten-Muster, getrennt von der großen Basisdatei — kürzer, gezielter pflegbar. |
...
| **`folien-lektion.css`** | Nur bei **`body.folien-lektion`**: Folien-Spalten, Karten, Diagramme, ADV-Kopf-Logik; **keine** Nav-/Chrome-Overrides | **Nach** `base.css` und `subpage.css`; Rahmen-Tokens **`--advanced-border-light`** / **`--advanced-border-dark`** aus **`base.css`**. |
| **`index.css`** | Startseiten-/Kartenraster (`index.html`), falls eingebunden | Breakpoints an den **Projekt-Kanon** anlehnen (siehe unten). |

### CSS: Schichten, Body-Basis, Breakpoints

- **Typische `<link>`-Reihenfolge:** `base.css` → `subpage.css` → bei Bedarf **`folien-lektion.css`** oder **`doc-a4.css`** (Details Checkliste §7).  
- **Globale `body`-Basis:** In **`subpage.css`** sind **`padding`**, **`max-width: 1400px`** und **`margin: 0 auto`** für **Standard-Unterseiten** zentral definiert. Seiten mit **voller Inhaltsbreite** (z. B. Landkarte, Standards, CTFL-Baum, ausgewählte HA) überschreiben lokal mit **`body { max-width: none; margin: 0; }`**; **`font-size`** am `body` bleibt bei Bedarf in **seitenspezifischen** `<style>`-Blöcken.  
- **Breakpoints (Kanon im Projekt):** **`720px`** — schmale Viewports / „mobil“ (u. a. Folien, Startseiten-Kartenraster); **`960px`** — Umstellungen mehrspaltiger Layouts (z. B. Folien-Spalten, HA3); **`1400px`** — typische **maximale Inhaltsbreite** der Standard-Unterseiten. Neue **`@media`**-Regeln **daran ausrichten**, keine willkürlichen Abweichungen ohne fachlichen Grund.  
- **Folien:** Die konsolidierte ADV-Kopf-Logik gilt unter **`.folien-lektion`** für alle Lektionen (L1–L8); Details der Umsetzung in **`folien-lektion.css`**, Token-Definitionen in **`base.css`**.
