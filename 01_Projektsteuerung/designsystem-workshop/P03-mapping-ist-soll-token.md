...
|-----|------|-------|---------|---------------|--------------|
| `.tag--ctfl` (Farben in `assets/css/base.css`; Hülle/Maße ggf. seitenlokal) | **C-SYLLABUS** | `--chip-syllabus-bg`, `--chip-syllabus-fg` (alias zu bestehenden `--ctfl-bg` / Textfarbe) | Web, Chips | Die grünliche Kennzeichnung für **Prüfungs-/Syllabus-Stoff** wird überall gleich benannt und lässt sich zentral feinjustieren. | `pages/01-lerninhalte/01-landkarte.html` — Chip „CTFL“ / Kap.-Tags in der Landkarte |
| `.tag--praxis` | **C-PRAXIS** | `--chip-praxis-bg`, `--chip-praxis-fg` | Web, Chips | **Berufs-/Praxis**-Bezug ist optisch einheitlich und klar von Syllabus-Chips getrennt. Redaktion Landkarte: **kurz**, **Ausführlich (Kürzel)** — [P04-04](P04-04-ctfl-lernwebsite-spec.md) **A.0.1b**. | `pages/01-lerninhalte/01-landkarte.html` — Chips „Praxis“ |
| `.tag--advanced` | **C-ADV** | `--chip-advanced-bg`, `--chip-advanced-fg` | Web, Chips | **Vertiefung/Advanced** teilt sich eine erkennbare Farbfamilie (Normen, K3-Hinweise). | `pages/01-lerninhalte/01-landkarte.html` — „Advanced“-Chips |
...
| `.entry-card*`, `.mod-card`, HA-`.card-grid` | **P-KARTEN** | `--card-bg`, `--card-border`, `--card-shadow` | `index` + Module | **Karten** (Einstieg, Module, HA-Web) haben **ein** Raster aus Fläche, Rand und Hover. | `index.html` — Einstiegskarten; `pages/01-lerninhalte/01-landkarte.html` — Modul-Karten |
| `.tag-row`, `.tag`, Landkarten-Chips | **P-CHIPS** | *gemeinsam mit C-\** `--chip-*` | Web | **Chip-Zeilen** (nicht nur Farbe der Domäne) haben einheitliche **Höhe, Abstand, Radius**. | `pages/01-lerninhalte/01-landkarte.html` — `tag-group` / Chips |
...
| `.top-nav-zone`, `.main-nav`, `.skip-link` | **P-CHROME** | `--nav-bg`, `--nav-link`, `--nav-rhythm` (teilw. schon `--nav-vertical-rhythm`) | Global | **Kopf-Navigation** und Skip-Link bleiben **an einem Token-Set** hängend anpassbar. | `assets/css/base.css` + alle Seiten mit **mehrzeiliger** Kopf-Nav (**Lernmaterial** / **Hausaufgaben** / **Lernzusammenfassung**) |
