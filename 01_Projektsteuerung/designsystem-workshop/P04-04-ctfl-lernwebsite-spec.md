...
Letzte Aktualisierung: 2026-04-03 — **A.0.1b** nachgezogen: Kap. **4** ohne **`~Kap. 4`**-Sammelgruppe (**KFG** unter **4.3.2**, Praxis **Überdeckungsanalyse** / **Äquivalenzklassentabelle**); Leit-Header **Kap. 1–3** vs. **`section__sub--emph`** Kap. **4**; zuvor: Landkarte Kap. **4** TOC, **`.ref`-Semantik**, Kap. **2.2** Unterzeilen, Kap. **3** + **A.0.1c**  
...
- **Kap. 3:** **3.1** „Grundlagen des statischen Tests“; Syllabus-Chips **3.1.1–3.1.3**; zusätzlich **„Statische Analyse“** als **C-SYLLABUS** mit **`ref` 3.1** (siehe **`.ref`-Semantik** oben). **3.2** „Feedback- und Reviewprozess“ (**3.2.1–3.2.5**), **3.2.4** inkl. Review-Arten einzeln. **Kein** MS-Badge im Kapitelheader (Lehrplan). **Leituntertitel** (didaktisch): z. B. **„Wie prüfen, ohne Software auszuführen?“** — **`section__sub--landkarte-leit`** im **Leit**-Header (wie Kap. **1** / **2**).
- **Kap. 4:** Kapitelüberschrift Lehrplan „Testanalyse und -entwurf“; **Reihenfolge** wie TOC: **4.1** „Testverfahren im Überblick“ (**FL-4.1.1**), **4.2** Black-Box (**4.2.1–4.2.4**; ergänzend Syllabus-Chips **Entscheidungstabelle**, **Zustandsdiagramm**, **Zustandstabelle** am passenden Anker), **C-PRAXIS**-Chip **Äquivalenzklassentabelle** mit **`ms-mark`**, **ohne** Syllabus-**.ref** (kein Lehrplan-Lemma); **4.3** White-Box (**4.3.1–4.3.3**), **KFG** mit **`ref` 4.3.2**, **C-PRAXIS** **Überdeckungsanalyse** mit **MS** (ohne **`.ref`**); **4.4** / **4.5** mit Lehrplan-Überschriften; **4.5**-Praxis z. B. **Behavior-Driven Development (BDD)**, **Gherkin**. **Langform vor Kürzel** bei solchen Praxis-Chips wo auf der Landkarte umgesetzt (z. B. **BDD** unter **4.5**, **Continuous Integration / Delivery (CI/CD)** unter Kap. **2**). **Keine** Sammelgruppe **„Testmodelle & Hilfsmittel“** / **`~Kap. 4`**.
- **Header mit MS-Badge:** **`section__title-cluster`** umschließt **Titel** (inkl. **`.ref`**) und **`ms-badge`**, `flex-wrap: nowrap`, damit das Badge nicht allein umbricht. **Leituntertitel Kap. 1–3:** **`section__header--landkarte-leit`** mit **`section__header-row`** (erste Zeile: Kap.-Nummer + Cluster; zweite Zeile: zentrierter Leitsatz **`section__sub--landkarte-leit`**) — vermeidet **drei** optische Zeilen durch Flex-Umbruch. **Kap. 4:** Leitsatz **„Wie werden Testfälle entworfen?“** als **`section__sub--emph`** (volle Breite unter dem Titelblock, **zentriert**, **fett**, kursiv). **Kap. 5–6:** schlichter **`section__sub`** im Header (kein Leit-Zweizeiler).
