...
Letzte Aktualisierung: 2026-04-03 — **A.0.1b/c:** Landkarte Kap. **6** (Syllabus **6.1**/**6.2**, Nutzen/Risiken); **C-PRAXIS** kurz, Konvention **Ausführlich (Kürzel)**; **Smoke-Tests** **C-SYLLABUS** **`ref` 5.1.7** (Kap. **5.1**) + **C-PRAXIS** Build-Gate (Kap. **2**); **SonarQube (Werkzeug, Kap. 6)** (Kap. **3**); **A.0.1c** Leserhinweis Doppelrolle; **`base.css`** `.tags + .tags`. Zuvor Landkarten-**Kopf** Kap. **1–6**  
...
- **`.ref` auf Syllabus-Chips (Semantik):** Die Zahl in **`.ref`** ist der **Syllabus-Anker** (Abschnitt oder Unterabschnitt), zu dem der Chip **inhaltlich** gehört. **Nicht** jeder Chip hat eine **eigene Zeile** im Inhaltsverzeichnis — das ist **absichtlich** und entspricht z. B. den **sieben Grundsätzen** unter **1.3** (alle **`ref` 1.3**), mehreren Review-Arten unter **3.2.4**, oder dem Begriff **„Statische Analyse“** mit **`ref` 3.1** (Inhalt von Kap. **3** / Abschnitt **3.1**, ohne eigene TOC-Zeile neben **3.1.1–3.1.3**). **Ausnahme / Kennzeichnung:** Thematische Gruppen **ohne** TOC-Nummer können redaktionell mit **`~…`** markiert werden, **falls** nötig — **Kap. 4** der Landkarte hat **keine** solche Sammelgruppe; z. B. **Kontrollflussgraph (KFG)** steht als **C-SYLLABUS** unter **`ref` 4.3.2**.
- **Kap. 2:** Kapitelüberschrift redaktionell „Testen während des SDLC“; **2.1** „Testen im Kontext eines SDLC“ mit **2.1.1–2.1.6** in TOC-Reihenfolge; **2.2** **eine** `tag-group` „Teststufen und Testarten“ **`ref` 2.2** mit Unterzeilen **`.tag-group__subtitle`** (**2.2.1 / 2.2.2 / 2.2.3**, Stil wie „Lernmodule“); **2.3** Wartungstest.
- **Kap. 3:** **3.1** „Grundlagen des statischen Tests“; Syllabus-Chips **3.1.1–3.1.3**; zusätzlich **„Statische Analyse“** als **C-SYLLABUS** mit **`ref` 3.1** (siehe **`.ref`-Semantik** oben). **C-PRAXIS** z. B. **Linting**; **SonarQube (Werkzeug, Kap. 6)**. **3.2** „Feedback- und Reviewprozess“ (**3.2.1–3.2.5**), **3.2.4** inkl. Review-Arten einzeln. **Kein** MS-Badge im Kapitelheader (Lehrplan). **Leituntertitel** z. B. **„Wie prüfen, ohne Software auszuführen?“** — **`section__sub--landkarte-leit`** (gleicher Kopf wie alle Kap.).
- **Kap. 4:** Kapitelüberschrift Lehrplan „Testanalyse und -entwurf“; **Reihenfolge** wie TOC: **4.1** „Testverfahren im Überblick“ (**FL-4.1.1**), **4.2** Black-Box (**4.2.1–4.2.4**; ergänzend Syllabus-Chips **Entscheidungstabelle**, **Zustandsdiagramm**, **Zustandstabelle** am passenden Anker), **C-PRAXIS**-Chip **Äquivalenzklassentabelle** mit **`ms-mark`**, **ohne** Syllabus-**.ref** (kein Lehrplan-Lemma); **4.3** White-Box (**4.3.1–4.3.3**), **KFG** mit **`ref` 4.3.2**, **C-PRAXIS** **Überdeckungsanalyse** mit **MS** (ohne **`.ref`**); **4.4** / **4.5** mit Lehrplan-Überschriften; **4.5**-Praxis z. B. **Behavior-Driven Development (BDD)**, **Gherkin**. **Keine** Sammelgruppe **„Testmodelle & Hilfsmittel“** / **`~Kap. 4`**.
- **Kap. 5:** **`tag-group__title`** = Lehrplan-Überschriften **5.1**–**5.5**; **C-SYLLABUS** zu **5.1.1–5.1.7** und **5.2.1–5.2.4** **wortgleich** Unterüberschriften (DE v4.0.2); **5.1** zusätzlich **C-SYLLABUS**-Chip **Smoke-Tests** mit **`ref` 5.1.7** (Lehrplan: u. a. Testquadranten **5.1.7**; Erwähnung auch unter **5.1.3**) **unmittelbar nach** dem TOC-Chip **5.1.7**; **parallel** unter **Kap. 2** **Smoke-Tests (Build-Gate)** als **C-PRAXIS**. **5.3** eine Gruppe mit **drei** TOC-Chips **5.3.1**–**5.3.3**; **5.4** **Konfigurationsmanagement** **`ref` 5.4**; **5.5** u. a. **FL-5.5.1**-Wortlaut **`ref` 5.5.1**.
- **C-PRAXIS** auf der Landkarte: **kurz**, **kein** Erklärsatz mit **„—“**; Konvention **Ausführlich (Kürzel)** wo üblich (z. B. **CI/CD**, **DoD**, **UAT**, **PR**).
- **Kap. 6:** **`tag-group__title`** = TOC **6.1** / **6.2**; **6.1** mehrere **C-SYLLABUS**-Chips (Stichpunkt-Kerne aus Lehrplan **6.1**, jeweils **`ref` 6.1**); **6.2** mit **`.tag-group__subtitle`** *Potenzielle Nutzen* / *Potenzielle Risiken*, kompakte **C-SYLLABUS**-Chips zu den Aufzählungen im Lehrplan (alle **`ref` 6.2**); danach **C-PRAXIS** / **C-ADV**.
- **Kapitelkopf (Kap. 1–6):** **`section__header--landkarte-leit`** → **Zeile 1** **`section__header-row`**: Kap.-Nr. + **`section__title-cluster`** (nur **`section__title`** + **`.ref`**). **Zeile 2** **`section__header-subrow`**: **`section__sub--landkarte-leit`** (zentriert) + **`ms-badge`** unten rechts (**Kap. 3** ohne Badge). Cluster: `flex-wrap: nowrap`. **`section__header-row`**: geringer **`gap`** Nummer↔Titel.
...
- **Derselbe Begriff, zwei Farben (bewusst):** Wenn **Kapitelkontext** wechselt, darf ein Lemma **noch einmal** vorkommen — mit **passender** Domäne. **Beispiele:** **Smoke-Tests** als **C-SYLLABUS** mit **`ref` 5.1.7** (Testquadranten, Kap. **5.1**) und **parallel** als **C-PRAXIS** **Smoke-Tests (Build-Gate)** (Kap. **2**). **SonarQube** als **C-PRAXIS** in Kap. **3** mit Verweis **(Werkzeug, Kap. 6)** und erneut in der **Werkzeugliste** Kap. **6**.
